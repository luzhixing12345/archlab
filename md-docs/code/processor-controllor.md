...
- B 型指令和 J 型指令除了需要按照说明重新拼接立即数 imm , 此外 imm 是没有 imm[0] 项的, 也就是拼接后需要左移一位
  ...
  > 这是考虑到 riscv 32I 指令集定长, 且 4 字节对齐, 所以转移指令只需要跳转到偶数地址即可, 扩大跳转范围
...
# 这里需要编译为 RSICV-32 平台的可执行文件, 所需两个编译选项指定 arch 和 ABI
```
...
0000000c <L1>:
  ...
  18:   0040076f                jal     a4,1c <L2>

0000001c <L2>:
  1c:   00c501a3                sb      a2,3(a0)
```
...
> RISCV 中 a0-a7 对应的是 r10-r17, 最后 mem[3] 的值是 0 和 20 以 2 为差距追逐 10 步后的 30 的结果
